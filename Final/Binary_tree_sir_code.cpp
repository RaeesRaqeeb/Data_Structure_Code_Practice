...
bool Is_one_child_tree(node* passed_root)
{
    // if(passed_root->root==NULL)return false;

    if(passed_root==NULL)return true;

    if(passed_root->left!= NULL && passed_root->right!=NULL)return false;

    bool left=true;
    bool right=true;

    if(passed_root->left == NULL && passed_root->right ==NULL) return true;

    if(passed_root->left==NULL)
            right=Is_one_child_tree(passed_root->right);
    if(passed_root->right==NULL)
            left=Is_one_child_tree(passed_root->left);
...
// root5->left->left->right=new node(95);
...
// cout<<"Maximum value in the binary tree is:"<<maximum_value(root5);

if(Is_one_child_tree(root4))
{
    cout<<"All parent nodes has one child:"<<endl;
}
else 
    cout<<"Not what we want"<<endl;
